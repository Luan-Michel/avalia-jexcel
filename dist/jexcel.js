...
console.log(ret);
        ...
        obj.createCellHeader = function(colNumber) {
            ...
            if (obj.options.columns[colNumber].title) {
                // Alteração para não mostrar as tags HTML no title
                var sub = obj.options.columns[colNumber].title.split('<a');
                sub[0] = sub[0].replace('<small>', '');
                sub[0] = sub[0].replace('</small>', '');
                obj.headers[colNumber].setAttribute('title', sub[0]);
                // Final alteração
            ...
            if (cell.classList.contains('readonly') == true) {
                ...
                if (obj.options.columns[x].editor) {
                    ...
                    if (obj.options.columns[x].type == 'hidden') {
                        ...
                        // Resolvendo erro de aparecer 'undefined' qndo não tem valor e dá double click na célula
                        if (value == 'undefined'){
                            editor.value = '';
                        } else {
                            editor.value = value;
